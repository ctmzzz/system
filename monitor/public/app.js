...
function initServerControl() {
    ...
    const remoteLabel = document.getElementById('serverRemoteLabel');
    const startBtn = document.getElementById('startServerBtn');
    const stopBtn = document.getElementById('stopServerBtn');

    if (remoteLabel) {
        remoteLabel.textContent = '远程监控';
    }
    ...
    async function updateServerStatus() {
        try {
            ...
            if (data.running) {
                statusEl.innerHTML = '<span class="status-dot online"></span> 远程: 运行中';
            } else {
                statusEl.innerHTML = '<span class="status-dot offline"></span> 远程: 未响应';
            }
        } catch (e) {
            statusEl.innerHTML = '<span class="status-dot offline"></span> 远程: 检测失败';
        }
    }

    if (startBtn) {
        startBtn.addEventListener('click', async () => {
            if (!confirm('确定要启动主服务器吗？')) return;
            try {
                const res = await fetch('/api/start-server', { method: 'POST' });
                const data = await res.json();
                alert(data.success ? '启动指令已发送' : '启动失败: ' + data.error);
            } catch (e) {
    ...
    if (stopBtn) {
        stopBtn.addEventListener('click', async () => {
            if (!confirm('确定要停止主服务器吗？')) return;
            try {
                const res = await fetch('/api/stop-server', { method: 'POST' });
                const data = await res.json();
                alert(data.success ? '停止指令已发送' : '停止失败: ' + data.error);
            } catch (e) {
